...
(defn update! [state dt dimensions options]
  (let [prev @state
        mouse (:mouse prev)
    ...
    (swap! state
           assoc :vehicles
           (map #(vehicle/borders
                  (vehicle/update (vehicle/separate % vehicles options) dt options)
...
(defn ^:export main []
  ...
  (let [state (atom (init-state))
    ...
    (core/animate #(update! state % [w h] @options-atom)
